...
public class LocationPlugin : MonoBehaviour
{
    ...
    public delegate void TimeChangedHandler(long _timeDifference);
    ...
    {
        GetPluginClass().Call("checkPermission");
    }

    public void StartLocationService(int _interval, int _fastestInterval, int _smallestDisplacement,string fname,string lname,long contactno,string start_time,string current_date)
    {
        StartLocationService(_interval, _fastestInterval, _smallestDisplacement, true, "ic_stat_my_location", "ic_stat_my_location",fname,lname,contactno,start_time,current_date);
    }

    /// <summary>
    /// Start google player location service
    /// </summary>
    public void StartLocationService(int _interval, int _fastestInterval, int _smallestDisplacement, bool _asForeground, String _foregroundIcon, String _notificationIcon,string fname,string lname,long contactno,string start_time,string current_date)
    {
        GetPluginClass().Call("startLocationService", _interval, _fastestInterval, _smallestDisplacement, _asForeground, _foregroundIcon, _notificationIcon,fname,lname,contactno,start_time,current_date);
    }
    ...
    private void OnLocationReceived(string _locationData)
    {
        ...
        location.provider = splittedLocationString[7]; //change it to time(later)
    ...
    {
        System.Globalization.CultureInfo cultureInfo = System.Globalization.CultureInfo.InvariantCulture;
        long duration = long.Parse(_timeDiff, cultureInfo);
