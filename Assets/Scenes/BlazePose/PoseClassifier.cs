...
public class PoseClassifier {
  ...
  public ClassificationResult classify(List<Vector3> landmarks) {
    ...
    SortedList maxDistances = new SortedList(maxDistanceTopK);
    ...
    foreach (KeyValuePair<PoseSample, float> sampleDistances in maxDistances) {
      ...
      if (meanDistances.Count > meanDistanceTopK) {
        meanDistances.GetByIndex(0);
      }
    }

    foreach (KeyValuePair<PoseSample, float> sampleDistances in meanDistances) {
      String className = sampleDistances.Key.getClassName();
